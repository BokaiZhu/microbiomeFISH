...
# Function
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
#' Produces probe combination (2 probes) that with optmized coverage and specificity for the target group.
...
Dual_optimization=function(df,target_group,usearch_location,reference_fasta,num_result=200){

  locations=unique(df$location)
  combination=combn(locations,2) # the combinations of the locations, combination of three
  ...
  for (i in c(1:dim(combination)[2])){ # each possible combinations of locations
    ...
    target_comb=subset(df,df$location==combination[1,i] | df$location==combination[2,i]) # a subset that only contains the target locations
  ...
  for (m in c(1: nrow(contain_comb))){
    ...
    Sys.sleep(1 / 100)
    # bar end
    write.fasta(contain_comb[m,c("target1","target2")],c("1","2"),"temp.fasta")
